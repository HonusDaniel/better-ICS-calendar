...
class R34ICS {
	...
	public $colors = array(
		...
		'offwhite' => '#f7f7f7',	// rgb(247,247,247)
		'silver' => '#dddddd',		// rgb(221,221,221)
		'neutral' => '#ababab', 	// rgb(171,171,171)
		'gray' => '#777777',		// rgb(119,119,119)
		'black' => '#191919',		// rgb(25,25,25)
	);
	...
	protected $carbon_path = '/vendors/ics-parser/vendor/nesbot/carbon/src/Carbon/Carbon.php';
	...
	public function __construct() {
		...
		// Set UTC as base timezone for wp_date()
	...
	public function color_key_html($args, $ics_data) {
		...
		if (count((array)$ics_data['colors']) > 1 && count((array)$ics_data['feed_titles']) > 1) {
			?>
			<div class="ics-calendar-color-key<?php if (!empty($args['legendinline'])) { echo ' inline'; } ?>">
				<?php
				foreach ((array)$ics_data['feed_titles'] as $feed_key => $feed_title) {
					$toggle_guid = r34ics_guid();
					?>
					<div class="ics-calendar-color-key-item" data-feed-key="<?php echo intval($feed_key); ?>">
						<label for="<?php echo esc_attr($toggle_guid); ?>"><input type="checkbox" id="<?php echo esc_attr($toggle_guid); ?>" class="ics-calendar-color-key-toggle" data-feed-key="<?php echo intval($feed_key); ?>" checked="checked" />
						<?php
						echo $feed_title;
	...
	public function days_of_week($format=null) {
		...
		switch ($format) {
			case 'min':
				$days_of_week = array(
					0 => $wp_locale->weekday_initial[$wp_locale->weekday[0]],
					1 => $wp_locale->weekday_initial[$wp_locale->weekday[1]],
					2 => $wp_locale->weekday_initial[$wp_locale->weekday[2]],
					3 => $wp_locale->weekday_initial[$wp_locale->weekday[3]],
					4 => $wp_locale->weekday_initial[$wp_locale->weekday[4]],
					5 => $wp_locale->weekday_initial[$wp_locale->weekday[5]],
					6 => $wp_locale->weekday_initial[$wp_locale->weekday[6]],
				);
				break;
			case 'short':
				$days_of_week = array(
					0 => $wp_locale->weekday_abbrev[$wp_locale->weekday[0]],
					1 => $wp_locale->weekday_abbrev[$wp_locale->weekday[1]],
					2 => $wp_locale->weekday_abbrev[$wp_locale->weekday[2]],
					3 => $wp_locale->weekday_abbrev[$wp_locale->weekday[3]],
					4 => $wp_locale->weekday_abbrev[$wp_locale->weekday[4]],
					5 => $wp_locale->weekday_abbrev[$wp_locale->weekday[5]],
					6 => $wp_locale->weekday_abbrev[$wp_locale->weekday[6]],
				);
				break;
			case 'full':
			default:
				$days_of_week = array(
					0 => $wp_locale->weekday[0],
					1 => $wp_locale->weekday[1],
					2 => $wp_locale->weekday[2],
					3 => $wp_locale->weekday[3],
					4 => $wp_locale->weekday[4],
					5 => $wp_locale->weekday[5],
					6 => $wp_locale->weekday[6],
				);
				break;
		}
		return $days_of_week;
	...
	public function display_calendar($args) {
		...
		if (empty($ics_data)) {
			...
			$ics_data['colors'] = !empty($color) ? r34ics_color_set(r34ics_space_pipe_explode($color)) : null;
			...
			foreach ((array)$ics_data['urls'] as $feed_key => $url) {
				...
				if ($ICal->hasEvents() && $ics_events = $ICal->eventsFromRange($range_start,$range_end)) {
					...
					foreach ((array)$ics_events as $event) {
						...
						// Time zone overrides ("float" events or manual overrides)
						elseif (strpos($event->dtstart, 'Z') === false) {
							$dtstart_date = substr($event->dtstart_array[1],0,8);
							$dtstart_time = substr($event->dtstart_array[1],9,6);
							$dtend_date = isset($event->dtend_array[1]) ? substr($event->dtend_array[1],0,8) : substr($event->dtend_tz,0,8);
							$dtend_time = isset($event->dtend_array[1]) ? substr($event->dtend_array[1],9,6) : substr($event->dtend_tz,9,6);
							$all_day = false;
						}
						// Get the calculated start date and time
						else {
						...
						if ($dtend_date != $dtstart_date) {
							...
							while ($loop_date <= $dtend_date) {
								...
								if ($all_day || ($loop_date != $dtstart_date && $loop_date != $dtend_date)) {
									$event_item['multiday_position'] = 'middle';
									if ($loop_date == $dtstart_date) {
										$event_item['multiday_position'] = 'first';
									}
									elseif ($loop_date == $dtend_date) {
										$event_item['multiday_position'] = 'last';
									}
								...
								elseif ($loop_date == $dtend_date) {
									...
									if ($dtend_time != '000000') {
										$event_item['sublabel'] = __('Končí v', 'r34ics') . ' ' . r34ics_time_format($dtend_time);
			...
			// @todo Refactor views to loop through earliest/latest range rather than expecting arrays in events list
	...
	public function event_description_html($args, $event, $classes=array(), $has_desc=null) {
		...
		<div class="<?php echo esc_attr(implode(' ', $descloc_class)); ?>">
			...
			if ($show_attachment && $attachment_is_image && $args['view'] == 'list') {
				?>
				<div class="attach attach_float"><?php echo $event['attach']; ?></div>
				<?php
			}
			...
			if (!empty($args['eventdesc'])) {
				if (!empty($event['eventdesc'])) {
					if (in_array($args['view'], array('list','widget')) && intval($args['eventdesc']) > 1) {
						$eventdesc_content .=	'<div class="descloc_toggle">' .
													'<div class="descloc_toggle_excerpt" aria-hidden="true">' .
														r34ics_maybe_make_clickable(wp_trim_words($event['eventdesc'], intval($args['eventdesc']))) .
													'</div>' .
													'<div class="descloc_toggle_full">' . r34ics_maybe_make_clickable($event['eventdesc']) . '</div>' .
												'</div>';
					}
					else {
						$eventdesc_content .=	r34ics_maybe_make_clickable($event['eventdesc']);
					}
				}
				if (empty($args['linktitles']) && empty($args['nolink']) && !empty($event['url'])) {
					$eventdesc_content .=	'<div class="eventdesc eventurl">' .
												'<a href="' . esc_url($event['url']) . '" target="_blank">' . esc_url($event['url']) . '</a>' .
											'</div>';
				}
			}
			// Show attachment after description if not list view
			if ($show_attachment && (!$attachment_is_image || $args['view'] != 'list')) {
				$eventdesc_content .=	'<div class="attach">' . $event['attach'] . '</div>';
			}
	...
	public function event_label_html($args, $event, $classes=array()) {
		...
		<span class="<?php echo esc_attr(implode(' ', $title_class)); ?>">
			<?php
			if (!empty($args['linktitles']) && empty($args['nolink']) && !empty($event['url'])) {
				echo '<a href="' . esc_url($event['url']) . '" target="_blank">';
			}
	...
	public function shortcode($atts) {
		...
		$args = array(
			...
			'startdate' => ($startdate == 'today' ? date('Ymd', current_time('timestamp')) : intval($startdate)),
			'title' => $title,
	...
	private function _load_parser() {
		if (!class_exists('Carbon\Carbon')) { include_once($this->carbon_path); }
